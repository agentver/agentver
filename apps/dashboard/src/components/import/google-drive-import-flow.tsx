...
export function GoogleDriveImportFlow() {
  ...
  const filesQuery = trpc.imports.listGoogleDriveFiles.useQuery(
  ...
  const importMutation = trpc.imports.importFromGoogleDrive.useMutation({
    onSuccess: (data) => {
  ...
  const handleScanCurrentFolder = useCallback(() => {
    ...
    const folderName = breadcrumbs[breadcrumbs.length - 1]?.name ?? 'My Drive'
  ...
  if (
    step === 'connect' ||
    (filesQuery.error?.message.includes('No Google account connected') && step === 'browse')
  ) {
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            You need to connect your Google account before importing from Google Drive.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Button asChild>
            <Link href="/settings/connections">Connect Google Account</Link>
          </Button>
        </CardContent>
      </Card>
    )
  ...
  if (step === 'browse') {
    ...
    return (
      <Card>
        <CardHeader>
          <CardTitle>Browse your Google Drive</CardTitle>
          <CardDescription>
